...
# download from https://github.com/aj-grant/robust-mvmr/blob/master/mvmr_functions.R
################################################################################
...
mvmr_lasso = function(bx, by, seby){
  ...
  return(list(thest = thest, v = v, lambda = cv.alas$lambda,
              est_post = th_post, se_post = se_post))
}
